...
def test_model_loading():
    ...
    assert isinstance(model, BertForSequenceClassification),  f"The loaded model is the wrong type: {type(model)}"
...
def test_text_preprocessing():
    ...
    assert encoding["input_ids"].shape == (1, 160), "input_ids has an incorrect form"
    assert encoding["attention_mask"].shape == (1, 160), "attention_mask has an incorrect form"
...
def test_model_inference():
    ...
    assert outputs.logits.shape == (1, len(LABELS)), f"Incorrect logit form: {outputs.logits.shape}"
